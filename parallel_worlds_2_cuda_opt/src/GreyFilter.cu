...
__host__ void GreyFilter::applyDevice(const unsigned char *deviceInput,
		unsigned char *deviceOutput, const unsigned int w, const unsigned int h) {
	this->prepareLaunch(w, h);
	greyKernel<<<this->grid,this->threads>>>(deviceInput, deviceOutput, w, h);
}
...
__host__ void GreyFilter::operator()(const unsigned char *input,unsigned char *output,
	...
	this->applyDevice(this->dInput, this->dOutput, w, h);
