...
__host__ void SobelFilter::applyDevice(const unsigned char *deviceInput,
		unsigned char *deviceOutput, const unsigned int w, const unsigned int h,
		const float c) {
	this->prepareLaunch(w, h);
	sobelKernel<<<this->grid,this->threads>>>(deviceInput, deviceOutput, w, h, c);
}
...
__host__ void SobelFilter::operator()(const unsigned char *input,unsigned char *output,
	...
	this->applyDevice(this->dInput, this->dOutput, w, h, c);
